...
export class Cybertruck {
	constructor(licensePlateImg) {
		...
		let W = this.width,
			...
			toFaces = a => new THREE.Vector3(a[0],a[1],a[2]);
		...
		let doorOutlineMat = new THREE.LineBasicMaterial({
            ...
            doorOutlineFLGeo = new ConvexGeometry(doorOutlineFLVertices);;
		...
		let doorOutlineFRVerticesArr = doorOutlineFLVerticesArr.map(flipXVertices),
			...
			doorOutlineFRGeo = new ConvexGeometry(doorOutlineFRVertices);
		...
		let doorOutlineMLVerticesArr = [
			...
			doorOutlineMLGeo = new ConvexGeometry(doorOutlineMLVertices);
		...
		let doorOutlineMRVerticesArr = doorOutlineMLVerticesArr.map(flipXVertices),
			...
			doorOutlineMRGeo = new ConvexGeometry(doorOutlineMRVertices);
		...
		let doorOutlineBLVerticesArr = [
			...
			doorOutlineBLGeo = new ConvexGeometry(doorOutlineBLVertices);
		...
		let doorOutlineBRVerticesArr = doorOutlineBLVerticesArr.map(flipXVertices),
			...
			doorOutlineBRGeo = new ConvexGeometry(doorOutlineBRVertices);
		...
		let fuelCapVerticesArr = [
			...
			fuelCapGeo = new ConvexGeometry(fuelCapVertices);
		...
		let windowMat = new THREE.MeshLambertMaterial({
			...
			topWindowGeo = new ConvexGeometry(topWindowVertices);
		...
		let topLightVerticesArr = [
			...
			topLightGeo = new ConvexGeometry(topLightVertices);
		...
		let slidingDoorMat = new THREE.MeshPhongMaterial({
			...
			slidingDoorGeo = new ConvexGeometry(slidingDoorVertices);
		...
		let sideWindowsVerticesArr = [
			...
			sideWindowsGeo = new ConvexGeometry(sideWindowsVertices);
		...
		let frontLightVerticesArr = [
			...
			frontLightGeo = new ConvexGeometry(frontLightVertices);
		...
		let lowerLightMat = new THREE.MeshBasicMaterial({
			...
			lowerLFrontLightGeo = new ConvexGeometry(lowerLFrontLightVertices);
		...
		let lowerRFrontLightVerticesArr = lowerLFrontLightVerticesArr.map(flipXVertices),
			...
			lowerRFrontLightGeo = new ConvexGeometry(lowerRFrontLightVertices);
		...
		let sideMat = new THREE.MeshStandardMaterial({
			...
			leftSideGeo = new ConvexGeometry(leftSideVertices);
		...
		let rightSideVerticesArr = leftSideVerticesArr.map(flipXVertices),
			...
			rightSideGeo = new ConvexGeometry(rightSideVertices);
		...
		let backVerticesArr = [
			...
			backGeo = new ConvexGeometry(backVertices),
		...
		let redLinesMat = new THREE.MeshStandardMaterial({
			...
			leftRedLinesGeo = new ConvexGeometry(leftRedLinesVertices);
		...
		let leftSmallBackLightVerticesArr = [
			...
			leftSmallBackLightGeo = new ConvexGeometry(leftSmallBackLightVertices);
		...
		let rightRedLinesVerticesArr = leftRedLinesVerticesArr.map(flipXVertices),
			...
			rightRedLinesGeo = new ConvexGeometry(rightRedLinesVertices);
		...
		let rightSmallBackLightVerticesArr = leftSmallBackLightVerticesArr.map(flipXVertices),
			...
			rightSmallBackLightGeo = new ConvexGeometry(rightSmallBackLightVertices);
		...
		let backBumperVerticesArr = [
			...
			backBumperGeo = new ConvexGeometry(backBumperVertices);
		...
		let frontBumperVerticesArr = [
			...
			frontBumperGeo = new ConvexGeometry(frontBumperVertices);
		...
		let supportMat = new THREE.MeshStandardMaterial({
			...
			frontAxleSupportGeo = new ConvexGeometry(frontAxleSupportVertices);
		...
		let backAxleSupportVerticesArr = [
			...
			backAxleSupportGeo = new ConvexGeometry(backAxleSupportVertices);
		...
		let bottomVerticesArr = [
			...
			bottomGeo = new ConvexGeometry(bottomVertices);
		...
		let spokeVerticesArr = [
			...
			spokeGeo = new ConvexGeometry(spokeVertices);
...
/*
	...
	updateNumCorners(cornersX){
		for(var i=1; i<cornersX.length; i++){ //Counts how much pieces the speedway has and add a false in cornersPassed for each one
	...
	hitCorner(cornersX, cornersZ, blockSize){
		...
		if((cornersX[this.cornerCount] >= -5) && (cornersX[this.cornerCount] <= 5)){
		  conditionX = (this.mesh.position.x >= -30 && this.mesh.position.x <=30);
		}else{
		  if(cornersX[this.cornerCount] > 0){
			conditionX = (this.mesh.position.x >= (cornersX[this.cornerCount] - blockSize)) && (this.mesh.position.x <= (cornersX[this.cornerCount] + blockSize));
		  }else{
			conditionX = ((this.mesh.position.x <= (cornersX[this.cornerCount] + blockSize)) && (this.mesh.position.x >= (cornersX[this.cornerCount] - blockSize)));
		  }
		}
	
		if((cornersZ[this.cornerCount] >= -5) && (cornersZ[this.cornerCount] <= 5)){
		  conditionZ = (this.mesh.position.z >= -30 && this.mesh.position.z <=30);
		}else{
		  if(cornersZ[this.cornerCount] > 0){
			conditionZ = (this.mesh.position.z >= (cornersZ[this.cornerCount] - blockSize)) && (this.mesh.position.z <= (cornersZ[this.cornerCount] + blockSize));
		  }else{
			conditionZ = (this.mesh.position.z <= (cornersZ[this.cornerCount] + blockSize)) && (this.mesh.position.z >= (cornersZ[this.cornerCount] - blockSize));
		  }
		}
	...
	hitFinishLine(xInitialBlock, zInitialBlock){
		...
		if((xInitialBlock >= -5) && (xInitialBlock <= 5)){
